...
class UNetBlockDown(nn.Module):
    def __init__(self, in_c, out_c, time_dim, attention):
        super().__init__()
...
class AcDiffusionUNet(nn.Module):
    def __init__(self, time_dim):
        super().__init__()
        print("AcDiffusionUNetV4 initializing...")
        ...
        c = (32*4, 32*9, 64*9, 128*9)
        self.conv_in = nn.Conv2d(3, c[0], 3, 1, 1)
        self.res1 = ResNetBlockDiff(c[0], time_dim)
        self.res2 = ResNetBlockDiff(c[0], time_dim)
        
        self.down1 = UNetBlockDown(c[0], c[1], time_dim, False)
        self.down2 = UNetBlockDown(c[1], c[2], time_dim, True)
        self.down3 = UNetBlockDown(c[2], c[3], time_dim, True)
        self.up1 = UNetBlockUp(c[3], c[2], time_dim, True)
        self.up2 = UNetBlockUp(c[2], c[1], time_dim, False)
        self.up3 = UNetBlockUp(c[1], c[0], time_dim, False)
