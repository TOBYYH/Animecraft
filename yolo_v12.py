...
try:
    ...
    from torch.nn.functional import scaled_dot_product_attention as sdpa
    logger.warning(f"FlashAttention is not available on this device. Using scaled_dot_product_attention instead: {e}")
...
class C2f(nn.Module):
    ...
    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5):
        ...
        self.cv2 = _Conv((2 + n) * self.c, c2, 1)  # optional act=FReLU(c2)
...
class C3(nn.Module):
    ...
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5):
        ...
        self.cv2 = _Conv(c1, c_, 1, 1)
        self.cv3 = _Conv(2 * c_, c2, 1)  # optional act=FReLU(c2)
...
class AAttn(nn.Module):
    ...
    def __init__(self, dim, num_heads, area=1):
        ...
        self.qk = _Conv(dim, all_head_dim * 2, 1, act=False)
        self.v = _Conv(dim, all_head_dim, 1, act=False)
        self.proj = _Conv(all_head_dim, dim, 1, act=False)

        self.pe = _Conv(all_head_dim, dim, 5, 1, 2, g=dim, act=False)
...
class ABlock(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=1.2, area=1):
        ...
        self.mlp = nn.Sequential(_Conv(dim, mlp_hidden_dim, 1), _Conv(mlp_hidden_dim, dim, 1, act=False))
...
class A2C2f(nn.Module):  
    ...
    def __init__(self, c1, c2, n=1, a2=True, area=1, residual=False, mlp_ratio=2.0, e=0.5, g=1, shortcut=True):
        ...
        self.cv1 = _Conv(c1, c_, 1, 1)
        self.cv2 = _Conv((1 + n) * c_, c2, 1)  # optional act=FReLU(c2)
...
class YoloV12mBackbone(nn.Module):
    def __init__(self, size=(9*64, 16*64), c=0):
        super().__init__()
        H, W = size
        self.nn = nn.Sequential(
            _Conv(3, 64, 4, 2, 1),
            _Conv(64, 128, 4, 2, 1, 2),
            C3k2(128, 256, 1, True, 0.25),
            _Conv(256, 256, 4, 2, 1, 4),
            C3k2(256, 512, 1, True, 0.25),
            _Conv(512, 512, 4, 2, 1),
            # A2C2f(512, 512, 2, True, 4),
            _Conv(512, 512, 4, 2, 1),
            # A2C2f(512, 512, 2, True, 1),
            _Conv(512, 64, 4, 2, 1),
            nn.Flatten()
        )
        self.D = nn.Sequential(
            nn.Linear(64*(H//64)*(W//64), 1),
            nn.Sigmoid()
        )
        if c == 0:
            self.E = None
        else:
            self.E = nn.Linear(64*(H//64)*(W//64), c)
    
    def forward(self, x):
        y = self.nn(x)
        if self.E is not None:
            return (self.D(y), self.E(y))
        else:
            return (self.D(y), None)
